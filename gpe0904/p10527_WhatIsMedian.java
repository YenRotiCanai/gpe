...
class p10527 {
    public static void main(String[] args){
        ...
        while(sc.hasNext()){
            arr[count] = sc.nextInt(); //把每一個 N 都塞進去 arr 裡面

            //開始插值
            for(int i = 1; i < count; i++){

                //System.out.println("count:"+count+", arrCount:"+arr[count]+", i:"+i+" arr[i]:"+arr[i]);

                if(arr[count] <= arr[i]){       //將arr裡面的一個個做比較,如果發現他比裡面其中一個小的話，就要換位子
                    int tmp = arr[i];           //先把原本 arr 裡面的東西給 tmp
                    arr[i] = arr[count];        //然後把他丟到他應該在的位置，這時候的i已經放新的值進去了
